...
1. [Constraint System (C1–C9)](#1-constraint-system)
2. [Workflow A: Catalog Item Creation (Steps 1-3)](#2-workflow-a-catalog-item-creation)
3. [Workflow B: Catalog Item Execution (Steps 4-11)](#3-workflow-b-catalog-item-execution)
4. [Validation Gates (G1–G6)](#4-validation-gates)
...
- Each question must offer **3-5 specific options** based on architectural patterns, roadmap context, or typical work categories
- Every question MUST include a final option: **"None of these / I'll specify: [free text]"**
- After each answer, the agent asks: **"Continue with next question, or proceed to catalog item proposal?"**
- User may choose to continue questioning or move to proposal at any point

#### Question Categories (Examples):

**Work Type Classification:**
> "What type of work is this?
> A) Principle-Driven (fixing architectural violation or advancing golden doc principle)
> B) Feature (new capability within established architecture)
> C) Bug Fix (correcting existing behavior)
> D) Infrastructure (tooling, logging, dev experience)
> E) None of these / I'll specify: [your answer]"

**Scope Boundaries:**
> "Which architectural layer should this touch?
> A) Extraction layer only
> B) Classification layer only
> C) Lens configuration only
> D) Multiple layers (will need careful scoping)
> E) None of these / I'll specify: [your answer]"

**Proof Strategy:**
> "How should this change be validated?
> A) Unit test checking specific behavior
> B) Integration test running entity through pipeline
> C) Database inspection for correct entity data
> D) CLI command validation
> E) None of these / I'll specify: [your answer]"
...
**Agent Action:**
Present each draft catalog item in the format:
...
- Write approved items to docs/progress/development-catalog.md
- Remove [DRAFT] prefix
- Confirm to user: "Catalog items [IDs] written to catalog. Session can be cleared. To execute, reference catalog item ID."
...
**Agent presents:**
"Catalog item [ID]. Will change [specific function in file]. Here's the micro-plan. Approve?"
...
**Purpose:**
Acknowledge that reality discovered during execution may legitimately invalidate parts of an approved micro-plan without invalidating the overall goal. This step provides a disciplined path to adapt without restarting from zero.
...
- **Broken Assumption** – what proved incorrect
- **New Facts** – evidence from code/reality
- **Proposed Delta** – minimal change to plan
- **Impact Statement** – confirmation that:
  - success criteria remain satisfied
  - scope limits (C3) remain intact
- **Updated Proof Strategy** (if needed)
...
1. **Ensure no new violations introduced elsewhere**
...
**Agent presents:**
"Complete. Catalog item [ID]. Changed [files]. Test passes: [test name]. Step 9 validation complete. Review?"
...
Proceed to Step 11 (Compounding)
...
**Recording:**
Capture answers in docs/progress/lessons-learned.md.
...
- Return to Step 4 (Code Reality Review)
- Update understanding of actual code
- Revise micro-plan based on reality, OR
- If scope grows beyond C3, stop and create new catalog item via Workflow A
...
**Action:**
- STOP.
- Mark current item as "blocked" or "needs split"
- Return to Workflow A to create new catalog items for discovered work
- Choose smaller scope OR address blocker first
...
- Return to Step 4 or Workflow A as appropriate
...
**Action:**
- STOP immediately. Golden docs win.
- Revert changes
- Re-read golden docs carefully
- Return to Workflow A to reshape catalog item, OR
- Return to Step 5 to revise micro-plan within current catalog item
...
- Re-read micro-plan
- Only implement what's explicitly in the plan
- No exceptions.
