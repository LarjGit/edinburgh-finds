...
- `core`: Entity identity and slug
- `location`: Geographic and address data
- `contact`: Contact methods and social media
- `hours`: Operating hours and schedules
- `amenities`: General amenities (parking, accessibility)
- `time_range`: Event date ranges

**Domain Modules** (lens-specific):
- `sports_facility`: Courts, pitches, equipment
- `fitness_facility`: Gym equipment, classes
- `wine_discovery`: Vintages, tasting notes (future vertical)
